...
const Tecnicos = () => {
    ...
    useEffect(() => {
        clienteAxios.get('/tecnicos').then(({ data }) => setTecnicos(data)).catch(console.log)
    }, [])

    const handleEliminar = async (id) => {
        const ok = await Swal.fire({
            title: '¿Dar de baja al técnico?', text: 'El perfil de este técnico será eliminado permanentemente.',
        ...
        if (ok.isConfirmed) {
            try {
                await clienteAxios.delete(`/tecnico/${id}`)
                setTecnicos(tecnicos.filter(t => t._id !== id))
                Swal.fire('¡Eliminado!', 'El técnico ha sido dado de baja.', 'success')
        ...
        <>
            ...
            <div className="pg-wrap">
                <div className="pg-header">
                    <div className="pg-title"><span>Módulo</span>Directorio de Técnicos</div>
                    <Link to="/dashboard/tecnicos/crear" className="pg-btn-add">+ Nuevo Técnico</Link>
                </div>
                <div className="pg-card">
                    <div className="pg-tbl-wrap">
                        <table className="pg-tbl">
                            <thead><tr>
                                <th>Cédula</th><th>Personal de Soporte</th>
                                <th>Contacto</th><th>Ubicación</th>
                                <th style={{textAlign:'center'}}>Acciones</th>
                            </tr></thead>
                            <tbody>
                                {tecnicos.map(t => (
                                    <tr key={t._id}>
                                        <td style={{fontWeight:700}}>{t.cedula}</td>
                                        <td style={{fontWeight:600,color:'#0F172A'}}>{t.nombre} {t.apellido}</td>
                                        <td><p className="pg-email">{t.email}</p><p className="pg-sub">{t.telefono}</p></td>
                                        <td>{t.ciudad}</td>
                                        <td><div style={{display:'flex',justifyContent:'center',gap:'0.25rem'}}>
                                            <Link to={`/dashboard/tecnicos/editar/${t._id}`} className="pg-edit">Editar</Link>
                                            <button onClick={() => handleEliminar(t._id)} className="pg-del">Eliminar</button>
                                        </div></td>
                                    </tr>
                                ))}
                                {tecnicos.length === 0 && <tr className="pg-empty"><td colSpan="5">No hay técnicos registrados en la cuadrilla.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </div>
